...
at::Tensor  fused_concat_embedding(std::vector<at::Tensor>& inputs, at::Tensor& weight, int64_t dim){
    ...
    std::vector<int64_t> out_tensor_shape = out_shape;
    out_tensor_shape.push_back(weight.size(1));
    at::Tensor output = at::empty(out_tensor_shape);
    ...
    int64_t numel = output.numel()/weight.size(1);
    for (int64_t ele_index=0; ele_index < numel; ele_index++){
        std::vector<int64_t> coord;
        int64_t remain_index = ele_index;
        int64_t cur_dim_index = 0;
        ...
        int indice_element = ((int*)inputs[tensor_index].const_data_ptr())[src_index];
        float* src_ptr = (float*)weight.const_data_ptr() + indice_element * weight.size(1);
        int64_t dst_index = 0;
        for (int i=0; i<ndim; i++){
            dst_index += coord[i] * output.stride(i);
        }
        // std::cout<< "dst index: " << dst_index << std::endl;
        float* dst_ptr = (float*)out_ptr + dst_index; 
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("fused_embedding", &fused_concat_embedding, "fused concat embedding");
}